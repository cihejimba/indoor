...
public class BeaconUtil {
	...
	/**
	 * RSSI = txPower + alfa*lg(d) + sigma
	 * 
	 * @param rssi
	 *            接受电平值
	 * @return 距离（米）
	 */
	public static double calculateAccuracy(double alfa,double sigma,double rssi) {
		
		double txPower = -51.0;
		
		if ((rssi == 0)||(alfa>0)||(alfa<-100)) {
			return -100; // if we cannot determine accuracy, return -1.
		}else {
			double dis = (rssi - txPower - sigma)/alfa;
			dis = Math.pow(10,dis);
			Log.i("iBeacon", "Dis double=>"+Double.toString(dis));
			return dis;
		}
	}
